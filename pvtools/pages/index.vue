<template>
  <b-container fluid>
    <b-row>
      ...
      <b-col>
        <iframe frameborder="0" scrolling="no" marginheight="0" marginwidth="0" width="200" height="100" type="text/html"
          ...
          <b-form>
            <b-form-group label="Adresse:">
              <b-input-group append="Straße, PLZ Stadt">
                <b-form-input v-model="inputAddressSearchString" placeholder="z.B. 50667 Köln" v-b-tooltip.hover
            ...
            <b-card bg-variant="light">
              <b-form-group label="Ausrichtung">
                <b-input-group append="° Grad Azimuth">
                  <b-input v-model.number="roofInput.aspect" type="number" min="-180" max="180" required v-b-tooltip.hover
                    title="0 = Süden, 90 = Westen, -90 = Osten" />
                </b-input-group>
              </b-form-group>
              <b-form-group label="Neigung">
                <b-input-group append="° Grad">
                  <b-input v-model.number="roofInput.angle" type="number" min="0" max="90" required v-b-tooltip.hover
                    title="0 = waargerecht, 90 = senkrecht" />
                </b-input-group>
              </b-form-group>
              <b-form-group label="Installierte Leistung">
                <b-input-group append="Wp">
                  <b-input v-model.number="roofInput.peakpower" min="1" type="number" required
              ...
              <b-list-group-item button :v-b-toggle="'roof' + roof.aspect + roof.angle + roof.peakpower">
                ...
                <b-button-group>
                  <b-button variant="primary"
                    ...
                    roofInput.horizon = roof.horizon
                    input.roofs = input.roofs.filter(roofEntry => !(roof.aspect == roofEntry.aspect && roof.angle == roofEntry.angle && roof.peakpower == roofEntry.peakpower && roof.horizon == roofEntry.horizon)) ">
    ...
    <b-overlay :show="isCalculating" rounded="sm">
      <b-row>
        <b-col>
          <div id="chartContainer">
            <Chart id="chart" v-if="displayData.length > 0" :labels="displayData.map(item => item.size)" :datasets="[
      ...
      <b-table v-if="displayData.length > 0" striped hover :items="displayData" :fields="tableFields" small
        responsive="sm">
        ...
        <template #row-details="row">
          <b-card>
            <b-table striped hover :items="[row.item]" :fields="[
            ...
            <b-table striped hover :items="roofsData" :fields="[
  ...
  methods: {
    ...
    buildQueryString(params) {
      ...
      let searchParams = {
        ...
        lat: params.lat,
        lon: params.lon,
        startyear: params.startyear || params.year || 2020,
        endyear: params.endyear || params.year || 2020,
        peakpower: params.peakpower,
        angle: params.angle,
        aspect: params.aspect
      }
      if (params.horizon != "") {
        searchParams["usehorizon"] = 1
        searchParams["userhorizon"] = params.horizon.match(/\d+/g).map(Number)
      }

      let url = new URL("https://re.jrc.ec.europa.eu/api/v5_2/seriescalc")
